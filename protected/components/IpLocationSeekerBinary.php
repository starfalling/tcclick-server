 ...
 * 基于项目 https://github.com/starfalling/php-IpLocationSeeker
 * @author york
 */
class IpLocationSeekerBinary {
	 ...
	 * 把数据转存到mysql数据库
	 */
	public function saveToTCClickMysql(){
		$sql = "drop table if exists {qqwry};";
		TCClick::app()->db->execute($sql);
		$sql = "create table {qqwry} (ip integer primary key, name varchar(255))
	...
	public function saveAsSqlite($filepath, $version=3, $use_china_province_name=true){
		include_once 'IpLocationSeekerTCClick.php';
		if(file_exists($filepath)) unlink($filepath); // 删掉旧库，重新建库
		$sqlite_seeker = new IpLocationSeekerTCClick($filepath, $version);
