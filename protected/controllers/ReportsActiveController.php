...
class ReportsActiveController extends Controller {
  ...
  private function calculateWeeklyActiveRate($week_start_date) {
    ...
    if($week_end_date > $today) { // 今天所在的这一周，直接取当前总激活设备数
      ...
      $all_devices_count = intval(TCClick::app()->db->query($sql)->fetchColumn());
      if($all_devices_count < $active_count) {
        // 这个数据不正确, 可能是因为 crontab 没有正常运行, 对该数据进行修复
        $sql = "select count(*) from {devices} where created_at<='{$week_end_date} 23:59:59'";
        $all_devices_count = intval(TCClick::app()->db->query($sql)->fetchColumn());
        $sql = "update {counter_daily} set all_devices_count={$all_devices_count} where date='{$week_end_date}'";
