...
include_once TCClick::app()->root_path . '/protected/analyze/Analyzer.php';
...
while(true){
	$result = TCClick::app()->cache->add(KEY_LOADING_CLIENT_ACTIVIES_LOCK, "locked");
	if($result) break;
	if(time() - $script_start_time > 150) exit;
	sleep(5);
}
...
if(!empty($rows)){
	...
	$sql = "insert into {client_activities_backup} select * from {client_activities}";
